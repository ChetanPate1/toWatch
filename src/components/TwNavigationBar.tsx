      ...
      return (
        <Tooltip
          ...
          <TooltipTrigger asChild>
            <NavLink
              to={item.to}
              className="relative text-center text-xl md:h-14 flex flex-row items-center justify-center mx-5 md:mx-0 md:my-2 group text-primary-foreground hover:bg-primary"
            >
              {({ isActive }) => (
                <>
                  {item.icon}
  ...
  if (token) {
    return (
      <TooltipProvider delayDuration={300}>
        <div className="flex flex-row md:flex-col justify-between fixed z-50 bottom-5 left-[5%] md:left-5 w-[90%] md:w-14 h-14 md:h-[90%] bg-white border-2 border-neutral-200 rounded-full dark:border-neutral-800 dark:bg-neutral-950" v-if="token">
