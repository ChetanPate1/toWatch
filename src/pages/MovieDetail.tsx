   ...
   const renderGenres = () => {
      if (data.genre) {
         return data.genre.split(', ').map((genre: string) => (
            <TwBadge key={genre} className="mr-1">{genre}</TwBadge>
         ));
      }
            ...
            <div className="grid grid-cols-2 gap-16 content-center">
               ...
               <div className="max-w-[555px]">
                  <h1 className="text-3xl font-bold text-white mt-2 relative">
                     <Link to="/movies" className="absolute -left-16 -top-2">
                        <TwCircleButton type="backward" />
                     </Link>

                     {data.title} <span className="text-sm text-slate-500">{data.year}</span>

                     <div className="text-sm text-slate-300 mt-2">{data.runtime} | {data.rated}</div>
                     <div>
                        {renderGenres()}
                     </div>
                  </h1>
